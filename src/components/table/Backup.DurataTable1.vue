...
import Checkbox from '../input/Checkbox.vue';
  ...
  /*
  function getGroupNameByIndex(index, groupData){
    if(Array.isArray(groupData[index])){
        return groupData[index][0].grpName;
    }else{
      if(groupData[index].stazione == StazioneId){
        return groupData[index].grpName;
      }
    }
    return "";
  }
*/
  ...
  function onSubmit() {
    ...
    ///RESET 
    /*rowsData.value.forEach( x => { 
      postEvStationData.value.payload[x.tmfield] = `0.0`;
      postEvStationData.value.payload[x.prgStep] = `0,1`;
