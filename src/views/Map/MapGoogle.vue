<template>
  ...
  <alert 
    message ="Idrosat coordinate not appropriate"
...
<script setup>
  ...
  const allDataReady = ref(false)

  async function getIdrosatGeo() {
    isIdrosatCoordinateValid.value = devicesStore.deviceData.coordinate.includes(',')
    let idrosatCoordinate

    if (isIdrosatCoordinateValid.value) {
      idrosatCoordinate = devicesStore.deviceData.coordinate.split(',')
      console.log('Idrosat Coordinates',idrosatCoordinate)
      idrosatMarkerPosition.value = {lat: parseFloat(idrosatCoordinate[0]), lng: parseFloat(idrosatCoordinate[1])}
    }
  }
  ...
  .background {
    @apply 
    bg-[#e6eff6] fixed h-screen w-screen z-0
  }
  .map-container {
    @apply 
